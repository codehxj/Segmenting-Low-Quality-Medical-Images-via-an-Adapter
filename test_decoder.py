...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, vis, txt, pad_mask):
        ...
        txt_pos = self.pos2d(C1, H1, W1)
        # txt_pos = self.pos1d(D, L)
        # reshape & permute
        vis = vis.reshape(B, C, -1).permute(2, 0, 1)
        txt = txt.reshape(B1, C1, -1).permute(2, 0, 1)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, vis, txt, vis_pos, txt_pos, pad_mask):
        ...
        # Cross-Attention
        vis2 = self.norm2(vis)
        # q1 = self.with_pos_embed(vis2, vis_pos)
        # k1 = self.with_pos_embed(txt, txt_pos)
...
# 定义一个卷积层，将输入特征从[1,256,56,56]转换为[1,512,56,56]
conv = nn.Conv2d(in_channels=256, out_channels=512, kernel_size=3, stride=1, padding=1)

# 定义一个下采样层（池化层），将输入特征从[1,512,56,56]转换为[1,512,14,14]
pool = nn.MaxPool2d(kernel_size=4, stride=4)

# 输入特征
input_feature = torch.randn(1, 256, 56, 56)

# 进行卷积操作
output_conv_feature = conv(input_feature)

# 进行下采样操作
output_feature = pool(output_conv_feature)

# 输出特征维度
print(output_feature.size())  # 输出：torch.Size([1, 512, 14, 14])




net = TransformerDecoder(3,512,8,2048,0.1,False)
fq = torch.randn(1,512,14,14)
b, c, h, w = fq.size()
# word = torch.randn(1,17,512)
word = torch.randn(1,512,14,14)
# pooled_feat = F.avg_pool2d(word, kernel_size=3, stride=3)

pad = torch.randn(1,17)
out = net(fq,word,pad)
out1 = out.reshape(b, c, h, w)
print(out.shape)
print(out1.shape)