...
class Projector2(nn.Module):
    def __init__(self, word_dim=16, in_dim=256, kernel_size=3):
        super().__init__()
        self.in_dim = in_dim
        ...
        self.concat_layer = nn.Conv2d(16+256, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, word):
        ...
        x = self.vis(x)
        B, C, H, W = x.size()
        word = nn.functional.interpolate(word, size=(H, W), mode='bilinear', align_corners=True)
        x = torch.cat((x, word), dim=1)
        out = self.concat_layer(x)
